...
> 具体源码可参看：https://github.com/vulnspy/ripstech-php-security-calendar-2017/blob/master/html/day11.php

**漏洞解析：** (上图代码第11行正则表达式应改为：'/O:\d:/')

题目考察对php反序列化函数的利用。在第10行 **loadData()** 函数中，我们发现了 **unserialize** 函数对传入的 **$data** 变量进行了反序列化。在反序列化前，对变量内容进行了判断，先不考虑绕过，跟踪一下变量，看看变量是否可控。

在代码 **第6行** ，调用了 **loadData()** 函数，`$data`变量来自于 **__construct()** 构造函数传入的变量。代码第32行，对 **Template** 类进行了实例化，并将 **cookie** 中键为'data'的数据作为初始化数据进行传入，所以`$data`数据我们可控。开始考虑绕过对传入数据的判断。
...
第二个if判断,匹配 字符串为 \'O:任意十进制:',将对象放入数组进行反序列化后，仍然能够匹配到，返回为空，考虑一下如何绕过正则匹配。
...
> 如何找到一个php函数对应的底层c代码呢？
>
> 一番搜索后我在此处发现了解决问题的办法：https://segmentfault.com/q/1010000003009774，主要是利用 http://lxr.php.net/ 这个域名来进行搜索
>
> ![image-20230414225843863](image-20230414225843863.png)

> 此外，关于PHP对应的c代码可以在php官方的github仓库下载，下面这个是php5.6.29的源码地址：https://github.com/php/php-src/tree/PHP-5.6.29/ext
>
> 关于如何读懂这些c代码，是待解决的一个问题
...
在PHP源码var_unserializer.c，对反序列化字符串进行处理，在代码568行对字符进行判断，并调用相应的函数进行处理，当字符为'O'时，调用 **yy13** 函数，在 **yy13** 函数中，对‘O‘字符的下一个字符进行判断，如果是':',则调用 **yy17** 函数,如果不是则调用 **yy3** 函数,直接return 0，结束反序列化。

接着看 **yy17** 函数。通过观察yybm[]数组可知，**第一个if判断是否为数字**，如果为数字则跳转到 **yy20** 函数。第二个if判断是否为 '+' 号，如果是 '+' 号则跳转到 **yy19** ，在 **yy19** 中，继续对 **+号** 后面的字符进行判断，如果为数字则跳转到 **yy20** ,如果不是则跳转到 **yy18** ，**y18** 最终跳转到 **yy3** ，退出反序列化流程。

**可以发现只要加号后面是数字，最终也可以跳转到yy20。也就是说 0:x 和 0:+x 这样形式的字符串最终都会跳到yy20正常的执行反序列化。**

绕过了过滤以后，接下来考虑怎样对反序列化进行利用，反序列化本质是将序列化的字符串还原成对应的类实例。在该过程中，我们可控的是序列化字符串的内容，也就是对应类中变量的值。我们无法直接调用类中的函数，但PHP在一定的条件下，会自动调用一些函数，该类函数，我们称为**魔术方法**。**通过可控的类变量，触发自动调用的魔术方法，若魔术方法中存在可利用点，则可形成反序列化漏洞。**
...
从上文可以学习到什么呢？若不谈论原理，只记忆粗浅的知识点的话，可以总结以下两点：

1. php反序列化数组时会将数组中的元素进行反序列化，在本题则序列化了一个类类型的元素。
2. 类类型的对象序列化时，可以将 O:x 写成 O:+x 的形式。 

> 启动镜像后需要更改一下权限，因为www-data用户不具有写入权限。
...
在上图代码 **第3行** ，对Cookie中的数据base64解码以后，进行了反序列化操作，该值可控。

接下来看一下代码触发条件。文件几个关键判断如下：

![8](8.png)

第一个if判断，可通过GET传递 **finish=任意值** 绕过 ，第二if判断是否有GET或者POST传参，并判断Referer是否为空，第四个if判断Referer是否为本站点。紧接着还有判断，如下图：

> 恕我直言，这种php代码加载着html代码的混合代码，真的像一坨屎山代码....
>
> 看的头皮发麻
>
> 这种代码的阅读不是看<? ?> 判断php代码是否结束，应该就是去掉这两个符号然后顺序阅读即可。据我推测，应该和python的阅读方法是一样的，根据行距判定子代码。
>
> 目前还不确定，因为开发经验少。
...
> 额，忘记了是在安装时进行漏洞利用。但我已经安装好了，懒得管了。

1. 访问首页

![image-20230415180816003](image-20230415180816003.png)



2.进入安装页面，管理员密码为 admin（注意，需要先在本地创建typecho数据库，否则无法连接。）

![image-20230415180850477](image-20230415180850477.png)



3. 安装成功

![image-20230415181205952](image-20230415181205952.png)
...
```
http://127.0.0.1:800/PHP_Audit/RedSun/DAY11/typecho-1.1-15.5.12-beta/admin/welcome.php
```

![image-20230415181244873](image-20230415181244873.png)
...
![image-20230415105426755](image-20230415105426755.png)
